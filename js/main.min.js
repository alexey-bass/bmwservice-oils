...
d=unique(d).sort();a=unique(a).sort(w);_populateDiv("brand",d);_populateDiv("sae",a);_populateDiv("tag");_populateDiv("size");_attachHandlers();_detectBestImgSize()};_postInit=function(){s=$("input.type-brand:checkbox ~ span.counter");m=$("input.type-sae:checkbox ~ span.counter");n=$("input.type-poly:radio ~ span.counter");p=$("input.type-clean:radio ~ span.counter");u=$("input.type-chemical:checkbox ~ span.counter");v=$("input.type-synth:checkbox ~ span.counter");_makeDbUsingFilters();g.updateCounters()};
_populateDiv=function(b,c){var d,a;a='<div class="header"><span class="title">';switch(b){case "brand":a+="\u0411\u0440\u0435\u043d\u0434\u044b";break;case "sae":a+="SAE";break;case "tag":a+="\u0424\u0438\u043b\u044c\u0442\u0440\u044b";break;case "size":a+="\u041a\u0430\u0440\u0442\u0438\u043d\u043a\u0438"}a+="</span>";"size"!==b&&(a+='<span class="link float-right select-none type-'+b+'" title="\u0421\u043d\u044f\u0442\u044c \u0432\u0441\u0435 \u0433\u0430\u043b\u043e\u0447\u043a\u0438">\u041e\u0447\u0438\u0441\u0442\u0438\u0442\u044c</span>');
...
q.length||r.length||_filterPolyEnabled()||_filterCleanEnabled()){var c;b="";for(c in l)b+=_makeResultItem(l[c]);$("#results").html(b);$("#show-all").addClass("hidden")}else $("#results").html(""),$("#show-all").removeClass("hidden")};g.updateCounters=function(){var b;s.html("");s.parent().addClass("disabled");for(b in f.brand)s.filter('[value="'+b+'"]').html(f.brand[b]).parent().removeClass("disabled");m.html("");m.parent().addClass("disabled");for(b in f.sae)m.filter('[value="'+b+'"]').html(f.sae[b]).parent().removeClass("disabled");
b=f.poly[0]+f.poly[1];n.filter("[value=poly-1]").html(f.poly[1]?f.poly[1]+" ("+Math.round(100*f.poly[1]/b)+"%)":"");n.filter("[value=poly-0]").html(f.poly[0]?f.poly[0]+" ("+Math.round(100*f.poly[0]/b)+"%)":"");p.filter("[value=clean-1]").html(f.clean[1]||"");p.filter("[value=clean-0]").html(f.clean[0]||"");u.html(f.chemical||"");v.html(f.synth||"")};_filterPolyEnabled=function(){return $("input.type-poly:checkbox").prop("checked")&&$("input[name=poly]:radio:checked").prop("checked")};_filterCleanEnabled=
